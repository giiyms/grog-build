...
pub async fn ask(model_id: &str, prompt: &str) -> Result<ConsultOutcome, ConsultError> {
    dispatch(model_id, prompt, ConsultKind::Ask).await
}
...
pub async fn provider_turn(model_id: &str, prompt: &str) -> Result<ConsultOutcome, ConsultError> {
    dispatch(model_id, prompt, ConsultKind::ProviderTurn).await
}
        ...
        .scope(Cell::new(true), async move {
            match (parsed.provider, kind) {
                (ProviderId::ClaudeBridge, ConsultKind::Ask) => {
                    let out = grog_claude_bridge::ask_claude(prompt, &parsed.model).await?;
                    Ok(ConsultOutcome {
                        text: out.text,
                ...
                (ProviderId::ClaudeBridge, ConsultKind::ProviderTurn) => {
                    let out = grog_claude_bridge::provider_turn(prompt, &parsed.model).await?;
                    Ok(ConsultOutcome {
                        text: out.text,
                        session_id: out.session_id,
                    })
                ...
                (ProviderId::Antigravity, ConsultKind::Ask) => {
                    let out = grog_antigravity::ask_agy(prompt, &parsed.model).await?;
                    Ok(ConsultOutcome {
                        text: out.text,
                        session_id: None,
                    })
                ...
                (ProviderId::Antigravity, ConsultKind::ProviderTurn) => {
                    let out = grog_antigravity::provider_turn(prompt, &parsed.model).await?;
                    Ok(ConsultOutcome {
                        text: out.text,
                        session_id: None,
                    })
                }
                (ProviderId::Codex, _) => consult_codex(&parsed.model, prompt).await,
...
async fn consult_codex(model: &str, prompt: &str) -> Result<ConsultOutcome, ConsultError> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        match grog_codex::consult_sync(&auth, &model, &prompt) {
            ...
            Err(grog_codex::ClientError::Http { status: 401, .. }) => {
                ...
                let out = grog_codex::consult_sync(&auth, &model, &prompt)?;
                Ok(ConsultOutcome {
                    text: out.text,
                    session_id: None,
                })
    ...
    async fn nested_consult_is_denied() {
        IN_CONSULT
            .scope(Cell::new(true), async {
                let err = ask("claude-opus-4-6", "hello").await.unwrap_err();
