...
//! cache via `GROK_CHANGELOG_OFFLINE` / `$GROG_HOME` / `$GROK_HOME`.
//!
//! `ChangelogManager::fetch()` returns a `Changelog` with optional markdown
//! and structured entries. Consumers pick the format they need:
...
impl ChangelogManager {
    ...
    fn from_env_home() -> Self {
        let home = std::env::var_os("GROG_HOME")
            .filter(|p| !p.is_empty())
    ...
    pub fn fetch(&self) -> Changelog {
        ...
        // seed a local cache via GROK_CHANGELOG_OFFLINE / GROG_HOME / GROK_HOME.
